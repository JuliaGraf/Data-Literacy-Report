...
\begin{figure}[ht]
    ...
    \caption{Beer price predictions from 1985 till 2023 using Gaussian process regression.}
    \label{figure_prediction}
\end{figure}

\begin{figure}[ht]
    % this figure made with plt.rcParams.update(bundles.icml2022(column="half”,usetex=False)) fig,ax = plt.subplots(nrows=1, ncols=1)
    \includegraphics{fig/gaussian_beer_price_CI.pdf} 
    \caption{Beer price prediction for the year 2023 using Gaussian process regression with an uncertainty level of one standard deviation.}
    \label{figure_prediction}
\end{figure}
